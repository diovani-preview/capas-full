...
const catalogData = categoryDefinitions.map((category, categoryIndex) => ({
    ...
    items: category.items.map((item, itemIndex) => ({
        ...
        /*
           Nova nomenclatura das capas e dos fundos internos:
           conteudo01, forum01, video01, atividade01, saibamais01
           ... até ...06.

           O arquivo img01 a img30 permanece como alternativa de segurança
           enquanto as novas imagens forem inseridas gradualmente no GitHub.
        */
        imageBase: `${ENVIRONMENT_IMAGE_PREFIXES[itemIndex]}${formatNumber(categoryIndex + 1)}`,
        legacyImageBase: `img${formatNumber(category.imageStart + itemIndex)}`,
...
const videoLibraries = {
    "mito-filosofia": [
        ...
        },
        {
            ...
            "shortDescription": "Percy descobre que é filho de Poseidon e que o mundo dos deuses gregos permanece ativo no presente.",
            "videoUrl": "",
            "embedUrl": "",
            "synopsis": "Percy descobre que é filho de Poseidon e que o mundo dos deuses gregos permanece ativo no presente. Ao ser envolvido no desaparecimento do raio de Zeus, parte com Annabeth e Grover em uma jornada para salvar sua mãe e impedir uma guerra entre os deuses.",
            "technicalTitle": "Percy Jackson & the Olympians: The Lightning Thief",
            "direction": "Chris Columbus",
            "cast": "Logan Lerman, Brandon T. Jackson, Alexandra Daddario, Pierce Brosnan, Sean Bean, Uma Thurman",
            "countryYear": "Estados Unidos, Reino Unido/2010",
            "publicAccessText": "Não foi localizada disponibilização integral confirmada em canal público autorizado.",
            "channel": "Não se aplica. Não foi localizado canal autorizado com o filme completo em domínio público.",
    ...
    "etica": [
        {
            ...
            "shortDescription": "Documentário e ficção se combinam para acompanhar uma comunidade Yanomami na Amazônia.",
            "videoUrl": "",
            "embedUrl": "",
            "synopsis": "Documentário e ficção se combinam para acompanhar uma comunidade Yanomami na Amazônia. O xamã Davi Kopenawa procura preservar os espíritos, os conhecimentos e as tradições de seu povo, enquanto o garimpo leva doenças, violência e ameaças ao território. A obra permite discutir ética ambiental, direitos indígenas, responsabilidade coletiva e respeito às diferentes formas de vida.",
            "technicalTitle": "A Última Floresta",
            "direction": "Luiz Bolognesi",
            "cast": "Davi Kopenawa Yanomami, Ehuana Yaira Yanomami, Pedrinho Yanomami, Joselino Yanomami e Nilson Wakari Yanomami",
            "countryYear": "Brasil/2021",
            "publicAccessText": "Não foi localizada disponibilização integral confirmada em canal público autorizado.",
            "channel": "Netflix e materiais oficiais da produtora Gullane",
            "sources": "Gullane — página oficial | Netflix Brasil",
            "pedagogicalRange": ""
        },
        {
            "number": 2,
    ...
    "filosofia-politica": [
        ...
        },
        {
            ...
            "available": true,
            "videoAvailable": false,
            "title": "Mandela: O Caminho para a Liberdade",
            "shortDescription": "A cinebiografia acompanha Nelson Mandela desde a juventude e a atuação contra o apartheid até a prisão, a libertação e a eleição presidencial.",
            "videoUrl": "",
            "embedUrl": "",
            "synopsis": "A cinebiografia acompanha Nelson Mandela desde a juventude e a atuação contra o apartheid até a prisão, a libertação e a eleição presidencial. A obra possibilita refletir sobre racismo institucional, resistência, violência política, prisão, democracia, negociação e reconciliação nacional.",
            "technicalTitle": "Mandela: Long Walk to Freedom",
            "direction": "Justin Chadwick",
            "cast": "Idris Elba, Naomie Harris, Tony Kgoroge e Riaad Moosa",
            "countryYear": "Reino Unido, África do Sul/2013",
            "publicAccessText": "Não foi localizada disponibilização integral confirmada em canal público autorizado.",
            "channel": "Pathé e plataformas de acesso licenciado",
            "sources": "Site oficial sul-africano | British Council — UK Films | Pathé Films",
            "pedagogicalRange": ""
        },
        {
            "number": 7,
...
function createEnvironmentEntryCard(category, definition) {
    ...
    loadRepositoryAsset(
        image,
        [item?.imageBase, item?.legacyImageBase],
        {
...
function createCoverCard(category, item) {
    ...
    card.dataset.legacyImageBase = item.legacyImageBase;
    ...
    fallback.innerHTML = `
        ...
        <small>Aguardando ${item.imageBase}</small>
    `;
    ...
    card.append(image, fallback, indicator);
...
function createVideoBadge(video) {
    ...
    } else if (video.videoAvailable) {
        badge.classList.add("public-video");
        badge.textContent = "Vídeo integral";
    } else {
...
function renderPublicAccessField(video) {
    ...
    if (!video.videoAvailable || !video.videoUrl) {
        return;
    }
    ...
    link.textContent = "Abrir vídeo integral no YouTube";
...
function openSelectedVideo(video) {
    ...
    if (video.videoAvailable && video.embedUrl && video.videoUrl) {
...
function openDetailPage(
    ...
    loadDetailImage([
        item.detailBackgroundBase,
        item.legacyImageBase
    ]);
